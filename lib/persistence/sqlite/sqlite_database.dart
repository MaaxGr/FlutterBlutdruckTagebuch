...
class SqliteDatabase {
  ...
  void init() async {
    ...
    this.database = openDatabase(join(await getDatabasesPath(), 'main.db'),
        onCreate: (db, version) {
      return db.execute(
  ...
  Future<void> insert(BloodPressureMeasurement measurement) async {
    ...
    db.insert('blood_pressure_measurement', measurement.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<void> delete(BloodPressureMeasurement measurement) async {
    final Database db = await database;
    db.delete('blood_pressure_measurement',
  ...
  Future<List<BloodPressureMeasurement>> getAll() async {
    ...
    final List<Map<String, dynamic>> mapList =
        await db.query('blood_pressure_measurement');
